                                    ...
                                    <div class="jd-profile-photo-card jd-bridge-profile-photo-card" x-show="bridgePhotoRows({!! $linkExpr !!}).length" x-init="$nextTick(() => { if(!{!! $linkExpr !!}.photo_scanned_once && bridgePhotoRows({!! $linkExpr !!}).length){ {!! $linkExpr !!}.photo_scanned_once = true; scanPhotos({!! $linkExpr !!}); } })" data-journalist-profile-photo-bridge>
                                        <div class="jd-profile-photo-head">
                                            ...
                                            <div class="jd-gallery-actions">
                                                <button type="button" class="jd-mini jd-mini-ghost" :disabled="{!! $linkExpr !!}.photo_busy" @click.stop="scanPhotos({!! $linkExpr !!})" data-journalist-action="load-bridge-folder-photos"><span x-show="{!! $linkExpr !!}.photo_busy===`scan`" class="jd-spin jd-spin-dark"></span><span x-text="{!! $linkExpr !!}.photo_busy===`scan` ? `Scanning...` : `Scan all sources`"></span></button>
                                                <label class="jd-mini jd-mini-indigo" :style="{!! $linkExpr !!}.photo_busy ? `opacity:.55;cursor:default` : ``" @click.stop data-journalist-action="upload-bridge-profile-photo"><span x-show="{!! $linkExpr !!}.photo_busy===`upload`" class="jd-spin"></span><span x-text="{!! $linkExpr !!}.photo_busy===`upload` ? `Uploading...` : `Upload`"></span><input type="file" accept="image/*" style="display:none" :disabled="{!! $linkExpr !!}.photo_busy" @change="uploadPhotoFile({!! $linkExpr !!}, $event, bridgeDefaultPhotoField({!! $linkExpr !!}))"></label>
                                            </div>
                                        </div>
                                                ...
                                                <template x-for="fieldRow in bridgePhotoRows({!! $linkExpr !!})" :key="fieldRow.key">
                                                    <div style="display:flex;gap:11px;align-items:flex-start;border:1px solid #e5e7eb;border-radius:10px;padding:9px 10px;background:#fff;">
                                                        <div style="width:46px;height:46px;border-radius:8px;background:#f1f5f9;flex-shrink:0;overflow:hidden;display:flex;align-items:center;justify-content:center;margin-top:17px;">
                                                            <template x-if="bridgePhotoUrl(fieldRow)"><img :src="bridgePhotoUrl(fieldRow)" alt="" loading="lazy" style="width:100%;height:100%;object-fit:cover;" x-on:error="$el.style.display=`none`"></template>
                                                            <template x-if="!bridgePhotoUrl(fieldRow)"><span style="color:#cbd5e1;font-size:16px;">+</span></template>
                                                        </div>
                                                    ...
                                                    <div class="jd-bridge-photo-row" x-show="fieldRow.is_photo_bridge" data-journalist-photo-bridge-row>
                                                        <div class="jd-photo-card" :class="bridgePhotoUrl(fieldRow) ? `` : `is-empty`">
                                                            ...
                                                            <div class="jd-photo-actions">
                                                                <button type="button" class="jd-mini jd-mini-ok" :disabled="{!! $linkExpr !!}.photo_busy || !bridgePhotoUrl(fieldRow)" :title="!bridgePhotoUrl(fieldRow) ? `Disabled because this Notion photo column has no URL to import.` : ({!! $linkExpr !!}.photo_busy ? disabledTitle({!! $linkExpr !!}) : `Import this Notion photo as the WordPress profile photo`)" @click.stop="importPhoto({!! $linkExpr !!}, bridgePhotoCandidate(fieldRow))"><span x-show="{!! $linkExpr !!}.photo_busy===fieldRow.key" class="jd-spin"></span><span>Use as WP Profile Photo</span></button>
                                                                <button type="button" class="jd-mini jd-mini-ghost" :disabled="{!! $linkExpr !!}.photo_busy || !bridgePhotoUrl(fieldRow)" :title="!bridgePhotoUrl(fieldRow) ? `Disabled because this row has no photo URL to save.` : ({!! $linkExpr !!}.photo_busy ? disabledTitle({!! $linkExpr !!}) : `Write this URL into ${fieldRow.notion_field || fieldRow.notion_label}`)" @click.stop="savePhotoToNotion({!! $linkExpr !!}, bridgePhotoCandidate(fieldRow), fieldRow.notion_field || fieldRow.notion_label)"><span x-show="{!! $linkExpr !!}.photo_busy===(`notion:` + fieldRow.key)" class="jd-spin jd-spin-dark"></span><span>Save URL to Notion</span></button>
                                                                <label class="jd-mini jd-mini-indigo" :title="{!! $linkExpr !!}.photo_busy ? disabledTitle({!! $linkExpr !!}) : `Upload a local image to WordPress and save the uploaded URL to this Notion column`" :style="{!! $linkExpr !!}.photo_busy ? `opacity:.55;cursor:default` : ``" @click.stop><span x-show="{!! $linkExpr !!}.photo_busy===`upload`" class="jd-spin"></span><span x-text="{!! $linkExpr !!}.photo_busy===`upload` ? `Uploading...` : `Upload to this column`"></span><input type="file" accept="image/*" style="display:none" :disabled="{!! $linkExpr !!}.photo_busy" @change="uploadPhotoFile({!! $linkExpr !!}, $event, fieldRow.notion_field || fieldRow.notion_label)"></label>
                                                            </div>
                                                        </div>
                                                    </div>
