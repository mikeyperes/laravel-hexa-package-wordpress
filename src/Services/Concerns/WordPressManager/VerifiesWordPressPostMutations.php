...
$formatData = static function (?array $state, array $verification = []): ?array {
    if (!$state) {
        return null;
    }
    return [
...
$writeRelationships = static function (int $id) use ($expectedTermChanges, $has, $payload): array {
    $errors = [];
    foreach ($expectedTermChanges as $taxonomy => $termIds) {
        $termResult = wp_set_object_terms($id, $termIds, $taxonomy, false);
        if (is_wp_error($termResult)) {
            $errors[] = "Taxonomy write failed for " . $taxonomy . ": " . $termResult->get_error_message();
        }
    }
    if ($has("featured_media")) {
        $featuredMediaId = (int) ($payload["featured_media"] ?? 0);
...
foreach ($fieldMap as $source => $destination) {
    ...
    $core[$destination] = $source === "slug" ? sanitize_title($value) : wp_slash($value);
}
...
if ($writeErrors !== [] || $stageMismatches !== []) {
    $rollbackResult = !$isCreate && $original ? $rollback($postId, $original) : null;
    $state = $isCreate ? $forceDraft($postId) : ($rollbackResult["state"] ?? $snapshot($postId));
    $problems = array_merge($writeErrors, array_keys($stageMismatches));
    $emit([
        "success" => false,
