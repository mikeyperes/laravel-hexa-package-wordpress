...
final class OutboundHttpSecurityTest extends TestCase
{
    ...
    {
        $calls = 0;
        $articleMetadata = new ArticleMetadataService(new SafeOutboundHttpClient(
            $this->guard(),
            function (OutboundHttpRequest $request) use (&$calls): OutboundHttpResponse {
                $calls++;
        ...
        $result = (new AuditableWordPressController(
            $this->createMock(WordPressService::class),
            $articleMetadata,
        ))->fetchMetadata('http://127.0.0.1/private');
    ...
    public function test_article_metadata_uses_pinned_tls_and_revalidates_redirects(): void
    {
        ...
        $articleMetadata = new ArticleMetadataService(new SafeOutboundHttpClient(
            $this->guard(),
            function (OutboundHttpRequest $request) use (&$requests): OutboundHttpResponse {
            $requests[] = $request;
        ...
        $result = (new AuditableWordPressController(
            $this->createMock(WordPressService::class),
            $articleMetadata,
        ))->fetchMetadata('https://public.example.org/article');

        $this->assertTrue($result['success']);
        $this->assertSame('Guarded article', $result['title']);
        $this->assertCount(1, $requests);
        $this->assertSame('public.example.org', $requests[0]->target->host);
        $this->assertPinnedRequest($requests[0], 1048576);

        $redirectRequests = [];
        $redirected = new ArticleMetadataService(new SafeOutboundHttpClient(
            $this->guard(),
            function (OutboundHttpRequest $request) use (&$redirectRequests): OutboundHttpResponse {
        ...
        $rejected = (new AuditableWordPressController(
            $this->createMock(WordPressService::class),
            $redirected,
        ))->fetchMetadata('https://public.example.org/article');

        $this->assertFalse($rejected['success']);
        $this->assertSame('Article metadata could not be fetched safely.', $rejected['message']);
    ...
    {
        $requests = [];
        $service = new AcfEducationMetadataService(new SafeOutboundHttpClient(
            $this->guard(),
            function (OutboundHttpRequest $request) use (&$requests): OutboundHttpResponse {
                $requests[] = $request;
    ...
    public function test_favicon_fallback_uses_verified_guarded_requests(): void
    {
        ...
        $this->assertCount(1, $requests);
        $this->assertPinnedRequest($requests[0], 512 * 1024);
    }
    ...
    private function assertPinnedRequest(OutboundHttpRequest $request, ?int $maxBytes = null): void
    {
        if ($maxBytes !== null) {
